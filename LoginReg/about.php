<!DOCTYPE html>
<html>

<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <meta name="generator" content="RocketCake">
  <title>Verifeye - About</title>
  <link rel="stylesheet" type="text/css" href="about_html.css"?v=<?php echo time(); ?>>
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=Oswald:wght@200..700&display=swap" rel="stylesheet">
</head>

<body>
  <div class="textstyle1" style="background-color: #001129;">
    <div id="container_53c17b29">
      <div id="container_53c17b29_padding">
        <div class="textstyle2">
          <div id="menu_3ed1db06" style="overflow: visible;">
            <div class="menuholder1" style="overflow: hidden;"><a href="javascript:void(0);">
                <div id="menuentry_657e55aa" class="menustyle1 menu_3ed1db06_mainMenuEntry mobileEntry">
                  <div class="menuentry_text1">
                    <span class="textstyle3">MENU ▼</span>
                  </div>
                </div>
              </a>
              <a href="index.php" style="text-decoration:none">
                <div id="menuentry_c145133" class="menustyle1 menu_3ed1db06_mainMenuEntry normalEntry">
                  <div class="menuentry_text2">
                    <span class="textstyle4">HOME</span>
                  </div>
                </div>
              </a>
              <a href="javascript:void(0);">
                <div id="menuentry_25d41ca0" class="menustyle2 menu_3ed1db06_mainMenuEntry normalEntry">
                  <div class="menuentry_text2">
                    <span class="textstyle5">ABOUT</span>
                  </div>
                </div>
              </a>
              <a href="courses.php" style="text-decoration:none">
                <div id="menuentry_a8bef60" class="menustyle3 menu_3ed1db06_mainMenuEntry normalEntry">
                  <div class="menuentry_text2">
                    <span class="textstyle4">COURSES</span>
                  </div>
                </div>
              </a>
              <a href="login.php" style="text-decoration:none">
                <div id="menuentry_a8bef60" class="menustyle4 menu_3ed1db06_mainMenuEntry normalEntry">
                  <div class="menuentry_text2">
                    <span class="textstyle4">LOGIN</span>
                  </div>
                </div>
              </a>

              <script type="text/javascript" src="rc_images/wsp_menu.js"></script>
              <script type="text/javascript">
                var js_menu_3ed1db06 = new wsp_menu('menu_3ed1db06', 'menu_3ed1db06', 10, null, true);

                js_menu_3ed1db06.createMenuForItem('menuentry_657e55aa', ["      <span class=\"textstyle6\">HOME</span> ", 'index.php', '',
                  "      <span class=\"textstyle6\">ABOUT</span> ", 'javascript:void(0);', '',
                  "      <span class=\"textstyle6\">COURSES</span> ", 'courses.php', '']);
                js_menu_3ed1db06.createMenuForItem('menuentry_c145133', []);
                js_menu_3ed1db06.createMenuForItem('menuentry_25d41ca0', []);
                js_menu_3ed1db06.createMenuForItem('menuentry_a8bef60', []);

              </script>
            </div>
            <div id="menu_3ed1db06_pane"
              style="position: absolute; left: 0px; top: 10px; overflow: visible; z-index: 10; visibility: hidden; display: none; width: 119px; white-space: nowrap;">
              <a href="index.php">
                <div id="menu_3ed1db06_entry" style="width: 99px;"> <span class="textstyle6">HOME</span> </div>
              </a><a href="javascript:void(0);">
                <div id="menu_3ed1db06_entry" style="width: 99px;"> <span class="textstyle6">ABOUT</span> </div>
              </a><a href="courses.php">
                <div id="menu_3ed1db06_entry" style="width: 99px;"> <span class="textstyle6">COURSES</span> </div>
              </a><a href="login.php">
                <div id="menu_3ed1db06_entry" style="width: 99px;"> <span class="textstyle6">LOGIN</span> </div>
              </a></div>
            <div id="menu_3ed1db06_pane"
              style="position: absolute; left: 0px; top: 10px; overflow: visible; z-index: 10; visibility: hidden; display: none; width: 119px; white-space: nowrap;">
              <a href="index.php">
                <div id="menu_3ed1db06_entry" style="width: 99px;"> <span class="textstyle6">HOME</span> </div>
              </a><a href="javascript:void(0);">
                <div id="menu_3ed1db06_entry" style="width: 99px;"> <span class="textstyle6">ABOUT</span> </div>
              </a><a href="courses.php">
                <div id="menu_3ed1db06_entry" style="width: 99px;"> <span class="textstyle6">COURSES</span> </div>
              </a><a href="login.php">
                <div id="menu_3ed1db06_entry" style="width: 99px;"> <span class="textstyle6">LOGIN</span> </div>
              </a></div>
          </div>
        </div>
        <div style="clear:both"></div>
      </div>
    </div>
  </div>
  <div id="headerab"><span class="textstyle7">VERIFEYE</span><span class="textstyle8">CREATORS</span></div><br><br><br><br><br><br><br>
  <div class="textstyle2">
    <div class="creatorspart">
      <div class="button_2ab67bc9">
        <div class="vcenterstyle1">
          <div class="vcenterstyle2">
            <div class="textstyle1">
              <div id="text_e1040de">
                <div class="textstyle1">
                  <span class="textstyle10">CHRISTINA MARTIROSYAN</span>
                </div>
              </div>
              <div id="text_6f97004c">
                <div class="textstyle1">
                  <span class="textstyle11">Programmer</span>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div class="button_2ab67bc9">
        <div class="vcenterstyle1">
          <div class="vcenterstyle2">
            <div class="textstyle1">
              <div id="text_35965677">
                <div class="textstyle1">
                  <span class="textstyle10">JEAN BADALYAN</span>
                </div>
              </div>
              <div id="text_19d801ec">
                <div class="textstyle1">
                  <span class="textstyle11">Programmer</span>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div class="button_2ab67bc9">
        <div class="vcenterstyle1">
          <div class="vcenterstyle2">
            <div class="textstyle1">
              <div id="text_3e1b6995">
                <div class="textstyle1">
                  <span class="textstyle10">MISHA VARDANYAN</span>
                </div>
              </div>
              <div id="text_91ac625">
                <div class="textstyle1">
                  <span class="textstyle11">Programmer</span>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div class="button_2ab67bc9 lastone">
        <div class="vcenterstyle1">
          <div class="vcenterstyle2">
            <div class="textstyle1">
              <div id="text_5ddaa22c">
                <div class="textstyle1">
                  <span class="textstyle10">POGHOS POGHOSYAN</span>
                </div>
              </div>
              <div id="text_48303f72">
                <div class="textstyle1">
                  <span class="textstyle11">Programmer</span>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    <br>
      <div class="button_2ab67bc9">
        <div class="vcenterstyle1">
          <div class="vcenterstyle2">
            <div class="textstyle1">
              <div id="text_5ddaa22c">
                <div class="textstyle1">
                  <span class="textstyle10">POGHOS POGHOSYAN</span>
                </div>
              </div>
              <div id="text_48303f72">
                <div class="textstyle1">
                  <span class="textstyle11">Programmer</span>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
  <div class="textstyle1" style="text-align: center;"><br><br><br><br>
    <span class="textstyle8">ABOUT</span><br><br><br>
  <span class="textstyle9">The Verifeye is an innovative application designed to estimate the time based on the position of the sun using an image of an object and its shadow. This project combines computer vision, image processing, and data retrieval techniques to provide users with an accurate estimation of the solar time.</span></div><br><br>
  <div class="textstyle2"><!--
    <span class="textstyle8">SPONSORS</span>
    <div class="creatorspart">
      <div id="button_2ab67bc9">
        <div class="vcenterstyle1">
          <div class="vcenterstyle2">
            <div class="textstyle1">
              <div id="text_e1040de">
                <div class="textstyle1">
                  <span class="textstyle10">CHRISTINA MARTIROSYAN</span>
                </div>
              </div>
              <div id="text_6f97004c">
                <div class="textstyle1">
                  <span class="textstyle11">Programmer</span>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div id="button_7fa21dec">
        <div class="vcenterstyle1">
          <div class="vcenterstyle2">
            <div class="textstyle1">
              <div id="text_35965677">
                <div class="textstyle1">
                  <span class="textstyle10">JEAN BADALYAN</span>
                </div>
              </div>
              <div id="text_19d801ec">
                <div class="textstyle1">
                  <span class="textstyle11">Programmer</span>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div id="button_16807a67">
        <div class="vcenterstyle1">
          <div class="vcenterstyle2">
            <div class="textstyle1">
              <div id="text_3e1b6995">
                <div class="textstyle1">
                  <span class="textstyle10">MISHA VARDANYAN</span>
                </div>
              </div>
              <div id="text_91ac625">
                <div class="textstyle1">
                  <span class="textstyle11">Programmer</span>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div id="button_409996f1">
        <div class="vcenterstyle1">
          <div class="vcenterstyle2">
            <div class="textstyle1">
              <div id="text_5ddaa22c">
                <div class="textstyle1">
                  <span class="textstyle10">POGHOS POGHOSYAN</span>
                </div>
              </div>
              <div id="text_48303f72">
                <div class="textstyle1">
                  <span class="textstyle11">Programmer</span>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div><br><br><br> ->

  <div id="sm-wrapper"></div>
</body>

</html>